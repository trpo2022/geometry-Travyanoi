...
CTEST(geometry_suite, Check_perimeter_of_triangle)
{
    ...
    test[6] = 0;
    test[7] = 0;
    double first_side = sqrt(SQR((test[3] - test[1])) + SQR((test[2] - test[0])));
    double second_side = sqrt(SQR((test[5] - test[3])) + SQR((test[4] - test[2])));
    double third_side = sqrt(SQR((test[5] - test[1])) + SQR((test[4] - test[0])));
...
CTEST(geometry_suite, Check_volume_of_triangle)
{
    ...
    const double exp = sqrt(half_perim * (half_perim - first_side) * (half_perim - second_side) * (half_perim - third_side));
